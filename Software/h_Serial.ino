      ...
      (serialPeriodicity == 's') || (serialPeriodicity == '!'))
  {
    switch (serialPage)
    {
      ...
      case 'X':   // eXcel Calibration report
        Console1.printf("ADC_VinRaw:\t%i\tVin:\t%06.3f\tADCIinRaw:\t%i\tIin:\t%06.3f\tADC_VoutRaw:\t%i\tVout:\t%06.3f\tADC_IoutRaw:\t%i\tIout:\t%06.3f\tPWM_SetVout:\t%04i\tSetVout:\t%06.3f\tPWM_SetIout:\t%04i\tSetIout:\t%06.3f\tPWM_Fan:\t%04i\n" , ADC_VinRaw, dashboard.Vin, ADC_IinRaw, dashboard.Iin, ADC_VoutRaw, dashboard.Vout, ADC_IoutRaw, dashboard.Iout, PWM_SetVout, dashboard.SetVout, PWM_SetIout, dashboard.SetIout, PWM_Fan);
        if (serialPeriodicity == '!') serialPage = 0; // One shot reset serial page.
        break;
      case 'J':   // Job Duration
      ...
      case '~':   // WiFi status report
        Console1.print(" RRSI= ");   Console1.print(WiFi.RSSI()); sprintf(charbuff, "dB, IP= %03d.%03d.%03d.%03d\n",  ip[0], ip[1], ip[2], ip[3]);  Console1.printf(charbuff);
        if (serialPeriodicity == '!') serialPage = 0; // One shot reset serial page.
        break;
